...
begin
  select coalesce(
      (select state from public.beer_pong_state where id = 'main'),
      '{}'::jsonb
    )
    into v_beer;

  select
    (select count(*)::integer from public.guests where status = 'confirmed')
    +
    (
  ...
  select coalesce(
      jsonb_agg(row_data order by rank_order desc, player_name asc),
      '[]'::jsonb
    )
