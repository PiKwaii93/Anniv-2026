...
export default function ChatHomeLink() {
  ...
  return <Link to="/chat" className="guest-activity chat-home-link" aria-label="Discussion des invités">
      <span className="guest-activity__icon" aria-hidden="true">💬</span>
      <div><h2>Discussion entre invités</h2><p>{error ? 'Ouvrir la discussion · connexion à vérifier' : data?.open === false ? 'Discussion en pause · les messages restent lisibles' : 'Écris un message à tous les invités.'}</p></div>
