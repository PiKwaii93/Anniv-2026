...
> = {
  preparation: {
    label: 'Préparation',
    detail:
      'Tout se met en place. Certains modules peuvent encore être masqués.',
  },
  live: {
    label: 'Soirée en cours',
    detail:
      'La soirée est lancée. Les modules évoluent en direct.',
  },
  ended: {
    label: 'Soirée terminée',
    detail:
      'Merci d’être passé. Les souvenirs restent accessibles ici.',
  },
}
...
function Home() {
  ...
  const loadStats = useCallback(async () => {
    ...
    if (icebergResult.error) {
      console.error(
        'Unable to load Home Iceberg stats:',
        icebergResult.error,
      )
    }
    ...
    if (bingoResult.error) {
      console.error(
        'Unable to load Home Bingo stats:',
        bingoResult.error,
      )
    }
    ...
    if (beerPongResult.error) {
      console.error(
        'Unable to load Home Beer Pong stats:',
  ...
  const beerPongStatus = useMemo(() => {
    if (stats.beerPong.championTeamId) {
      return 'Tournoi terminé · champion désigné'
    }

    if (stats.beerPong.draftValidated) {
      const teamCount =
        stats.beerPong.teams?.length ?? 0

      return `${teamCount} équipe${teamCount > 1 ? 's' : ''} · tournoi en cours`
    }

    if (
      (stats.beerPong.teams?.length ?? 0) > 0
    ) {
      return 'Draft prête à être lancée'
    }

    const playerCount =
      stats.beerPong.selectedPlayerIds
        ?.length ?? 0

    if (playerCount > 0) {
      return `${playerCount} joueur${playerCount > 1 ? 's' : ''} sélectionné${playerCount > 1 ? 's' : ''}`
    }
  ...
  const visibleModules = useMemo(() => {
    return publicModules
      ...
      .sort((left, right) => {
        if (
          left.key === settings.featuredModule
        ) {
          return -1
        }

        if (
          right.key === settings.featuredModule
        ) {
          return 1
        }
  ...
  return (
    <main className="home">
      ...
      <section
        className={`home-party-state home-party-state--${settings.phase}`}
        aria-label="État de la soirée"
      >
      ...
      <section className="modules" aria-label="Modules">
        {visibleModules.map((module) => {
          const featured =
            module.key === settings.featuredModule
        ...
        <Link
          to="/admin"
          className="module-card module-card--admin"
        >
          <div className="module-card__top">
