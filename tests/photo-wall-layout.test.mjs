...
test('TV wall uses continuous virtual scrolling and no page rotation', async () => {
  ...
  assert.match(masonrySource, /LOOP_END_PAUSE_MS/)
  assert.match(masonrySource, /scrollTop = 0/)
  ...
  assert.match(cssSource, /\.photo-hunt-screen\s*\{[\s\S]*position:\s*fixed/)
  assert.match(cssSource, /\.photo-hunt-screen\s*\{[\s\S]*height:\s*100dvh/)
})
