...
test('guest access controls are folded by default and open from their heading',async()=>{await render(true);const details=document.querySelector('#sessions');assert.equal(details.open,false);await click(details.querySelector('summary'));assert.equal(details.open,true);assert.ok(button('Déconnecter'))})
